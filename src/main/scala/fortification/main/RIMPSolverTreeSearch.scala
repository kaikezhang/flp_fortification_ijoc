...
object RIMPSolverTreeSearch {
  def main(args: Array[String]): Unit = {

    val (resultOut, logOut) = ResultOutput.createOutputPrinter("RIMPSolverTreeSearch")
    ...
    Console.withOut(outCapture) {
      ...
      for (node <- nodes; p <- ps; defendUnit <- defendUnits; attackUnit <- attackUnits) {
        ...
        val solverCuttingPlane = new RIMPImplicitEnumerateSolver(rimpInstance, instructor)
