...
import fortification.data.DefenderProblemInstanceSpecial
...
class RIMPCuttingPlaneSolver(val instance: RIMPInstance, val instructor: SolverInstructor) extends timer with TransportationCostsCalculator {
  import instance._
...
//        println("Valid no cut is added")
      ...
      for (j <- openLocationIndexes.diff(attacked).diff(fortifyLocs)) {
  ...
  def solve(): Option[RIMPSolution] = {
    ...
    try {
      ...
      val (solutionTrspCosts, attacked) = getTransptCosts(openLocationIndexes, bestSolution)
      
//      println("Empty defender attacked:", attacked)
      //      println(openIndexes)
      val fortify = openIndexes.map { j => cplex.boolVar("z_"+(j+1)) }

      val phi = cplex.numVar(0, Double.MaxValue, "phi")
      ...
      cplex.addGe(attacked.map { j => fortify(openIndexes.indexOf(j)) }.fold(cplex.numExpr)(cplex.sum), 1)
      ...
      if (cplex.solve()) {
        ...
        val attackInstance = new AttackerProblemInstance(instance.locationContext, openLocationIndexes, bestSolution, attackUnits)
...
//      val defenderInstance = new DefenderProblemInstanceP0(locationInstance, defenceUnits, attackUnits, w)
      val defenderInstance = new DefenderProblemInstanceSpecial(locationInstance, defenceUnits, attackUnits, w)
//      DefenderProblemInstanceSpecial
      
      
//      defenderInstance.randomFailRate 
      
      print(defenderInstance.randomFailRate)

      val rimpInstance = new RIMPInstance(defenderInstance, (0 until p).toSet)

      val instructor = SolverInstructor(gap = 0.0001)

      val solver = new RIMPCuttingPlaneSolver(rimpInstance, instructor)
