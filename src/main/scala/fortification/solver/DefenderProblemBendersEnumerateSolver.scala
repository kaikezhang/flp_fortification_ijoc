...
class DefenderProblemBendersEnumerateSolver(val instance: DefenderProblemInstance, val instructor: SolverInstructor) extends timer  {
  import instance._
  ...
  class LogicBendersEnumerateCutLazyConstraint(cplex: IloCplex, open: IndexedSeq[IloIntVar], fortify: IndexedSeq[IloIntVar], phi: IloNumVar) extends LazyConstraintCallback {
...
//        println(s"UpperBound:${upperBound}")
      ...
      val nonForityIndexes  = candidateLocationIndexes.toSet.diff(fortifyLocs)

      for (j <- nonOpenIndexes) {
        cut.addTerm(-solutionTrspCosts, open(j))
      }

      for (j <- nonForityIndexes) {
        cut.addTerm(-solutionTrspCosts, fortify(j))
      }      
  ...
  def solve(): Option[DefendProblemSolution] = {
    ...
    try {
      ...
      cplex.use(new LogicBendersEnumerateCutLazyConstraint(cplex, open, fortify, phi))
...
object DefenderProblemBendersEnumerateSolver {
  def main(args: Array[String]): Unit = {
    ...
    Console.withOut(outCapture) {
      ...
      val solver = new DefenderProblemBendersEnumerateSolver(defenderInstance, instructor)
